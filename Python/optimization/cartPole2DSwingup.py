...
class vdpCollocation():
  ...
  def vdpODE(self): # ODE rhs function and quadratures
    t, u, q, _, _ = self.problemSpecificParam()
    v = q[3]; w = q[2]; th = q[0]; x = q[1]
    m = 0.25
    mc = 1
    mup = 0.1
    muc = 0.4
    ...
    wdot = (g*np.sin(th) + (muc*np.sign(v)-u-m*l*w**2*np.sin(th)*np.cos(th))/(m+mc)-(mup*w)/(m*l))/(l*((4.0/3)-(m/(m+mc)*np.cos(th)**2)))
    vdot = (u + m*l*(w**2*np.sin(th)-wdot*np.cos(th)) - muc*np.sign(v))/(m+mc)
    xdot = vertcat(w,v,wdot,vdot)
    qdot = q[0]*q[0] + q[1]*q[1] + u*u
    f = Function('f', [t,q,u],[xdot, qdot])
  ...
  def stateBounds(self): # State bounds and initial guess 
    x_min =  [-np.pi, -1, -inf, -inf]
    x_max =  [ np.pi,  1, inf,  inf]
    xi_min = [ 0.0,  0.0, 0.0, 0.0]
    xi_max = [ 0.0,  0.0, 0.0, 0.0]
    xf_min = [ np.pi,  -1.0, 0.0, 0.0]
    xf_max = [ np.pi,  1.0, 0.0, 0.0]
  ...
  def plotSoln(self): # Plot the results
    ...
    plt.title("Cart-pole optimization")
    plt.xlabel('time')
    plt.legend(['th trajectory','x trajectory','u trajectory'])
    plt.grid()
    plt.show()
